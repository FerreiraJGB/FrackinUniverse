...
scannerLib2.images = {--planning to use image copies of tiles instead, somehow
							...
							if not self.colorCache[sType][cacheKey] then
								if sType == "cave" then
									if world.pointTileCollision(position) then
										self.colorCache[sType][cacheKey] = scannerLib2.colors.solid
									else
										self.colorCache[sType][cacheKey] = scannerLib2.colors.empty
									end
								elseif sType == "ore" then
									...
									if scannerLib2.colors[oreMod] then
										self.colorCache[sType][cacheKey] = scannerLib2.colors[oreMod]
									else
										self.colorCache[sType][cacheKey] = scannerLib2.colors.none
									end
								end
							end
						end
