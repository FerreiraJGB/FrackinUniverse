							...
							if not self.colorCache[sType][cacheKey] then
								if sType == "cave" then
									if world.pointTileCollision(position) then
										self.colorCache[sType][cacheKey] = scannerLib.colors[1].solid
									else
										self.colorCache[sType][cacheKey] = scannerLib.colors[1].empty
									end
...
function getLevel()
	parameter.level = config.getParameter("level")
end
