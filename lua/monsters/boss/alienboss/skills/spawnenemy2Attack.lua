...
--------------------------------------------------------------------------------
function spawnenemy2Attack.initialStateData()
  return {
    riseHeight = config.getParameter("spawnenemy2Attack.riseHeight"),
    riseSpeed = config.getParameter("spawnenemy2Attack.riseSpeed"),
    firing = false,
    skillTimer = 0,
    skillDuration = config.getParameter("spawnenemy2Attack.skillDuration"),
    angleCycle = config.getParameter("spawnenemy2Attack.angleCycle"),
    fireTimer = 0,
    fireInterval = config.getParameter("spawnenemy2Attack.fireInterval"),
    fireAngle = 0,
    maxFireAngle = config.getParameter("spawnenemy2Attack.maxFireAngle"),
    projectileCount = config.getParameter("spawnenemy2Attack.projectileCount"),
    winddownTimer = config.getParameter("spawnenemy2Attack.winddownTime")
  }
end

function spawnenemy2Attack.enter()
  if not hasTarget() or currentPhase() < 4 then return nil end

  return spawnenemy2Attack.initialStateData()
end


function spawnenemy2Attack.enterWith(args)
...
--------------------------------------------------------------------------------
function spawnenemy2Attack.enteringState(stateData)
  animator.setAnimationState("movement", "idle")

  monster.setActiveSkillName("spawnenemy2Attack")

  stateData.lastToSpawn = world.distance(self.spawnPosition, mcontroller.position())
end

--------------------------------------------------------------------------------
function spawnenemy2Attack.update(dt, stateData)
  ...
  --In firing position
    ...
    if stateData.skillTimer < stateData.skillDuration then
      ...
      if stateData.fireTimer <= 0 then
        ...
        spawnenemy2Attack.fire(angle, stateData.projectileCount)
    ...
    --Wind down floating to the ground before leaving the state
      ...
      if stateData.winddownTimer == config.getParameter("spawnenemy2Attack.winddownTime") then
...
function spawnenemy2Attack.fire(angle, count)
  local projectileType = config.getParameter("spawnenemy2Attack.projectile.type")
  local projectileConfig = config.getParameter("spawnenemy2Attack.projectile.config")

  if projectileConfig.power then
    projectileConfig.power = projectileConfig.power * root.evalFunction("monsterLevelPowerMultiplier", monster.level())
  end

  local innerRadius = config.getParameter("spawnenemy2Attack.projectile.innerRadius")
...
function spawnenemy2Attack.leavingState(stateData)
