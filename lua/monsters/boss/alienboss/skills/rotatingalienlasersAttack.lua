...
--------------------------------------------------------------------------------
function rotatingalienlasersAttack.initialStateData()
  return {
    riseHeight = config.getParameter("rotatingalienlasersAttack.riseHeight"),
    riseSpeed = config.getParameter("rotatingalienlasersAttack.riseSpeed"),
    firing = false,
    skillTimer = 0,
    skillDuration = config.getParameter("rotatingalienlasersAttack.skillDuration"),
    angleCycle = config.getParameter("rotatingalienlasersAttack.angleCycle"),
    fireTimer = 0,
    fireInterval = config.getParameter("rotatingalienlasersAttack.fireInterval"),
    fireAngle = 0,
    maxFireAngle = config.getParameter("rotatingalienlasersAttack.maxFireAngle"),
    projectileCount = config.getParameter("rotatingalienlasersAttack.projectileCount"),
    winddownTimer = config.getParameter("rotatingalienlasersAttack.winddownTime")
  }
end

function rotatingalienlasersAttack.enter()
  if not hasTarget() or currentPhase() < 4 then return nil end

  return rotatingalienlasersAttack.initialStateData()
end


function rotatingalienlasersAttack.enterWith(args)
...
--------------------------------------------------------------------------------
function rotatingalienlasersAttack.enteringState(stateData)
  animator.setAnimationState("movement", "idle")

  monster.setActiveSkillName("rotatingalienlasersAttack")

  stateData.lastToSpawn = world.distance(self.spawnPosition, mcontroller.position())
end

--------------------------------------------------------------------------------
function rotatingalienlasersAttack.update(dt, stateData)
  ...
  --In firing position
    ...
    if stateData.skillTimer < stateData.skillDuration then
      ...
      if stateData.fireTimer <= 0 then
        ...
        rotatingalienlasersAttack.fire(angle, stateData.projectileCount)
    ...
    --Wind down floating to the ground before leaving the state
      ...
      if stateData.winddownTimer == config.getParameter("rotatingalienlasersAttack.winddownTime") then
...
function rotatingalienlasersAttack.fire(angle, count)
  local projectileType = config.getParameter("rotatingalienlasersAttack.projectile.type")
  local projectileConfig = config.getParameter("rotatingalienlasersAttack.projectile.config")

  if projectileConfig.power then
    projectileConfig.power = projectileConfig.power * root.evalFunction("monsterLevelPowerMultiplier", monster.level())
  end

  local innerRadius = config.getParameter("rotatingalienlasersAttack.projectile.innerRadius")
...
function rotatingalienlasersAttack.leavingState(stateData)
