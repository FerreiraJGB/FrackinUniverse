...
--------------------------------------------------------------------------------
function spawnalienqueenminionAttack.initialStateData()
  return {
    riseHeight = config.getParameter("spawnalienqueenminionAttack.riseHeight"),
    riseSpeed = config.getParameter("spawnalienqueenminionAttack.riseSpeed"),
    firing = false,
    skillTimer = 0,
    skillDuration = config.getParameter("spawnalienqueenminionAttack.skillDuration"),
    angleCycle = config.getParameter("spawnalienqueenminionAttack.angleCycle"),
    fireTimer = 0,
    fireInterval = config.getParameter("spawnalienqueenminionAttack.fireInterval"),
    fireAngle = 0,
    maxFireAngle = config.getParameter("spawnalienqueenminionAttack.maxFireAngle"),
    projectileCount = config.getParameter("spawnalienqueenminionAttack.projectileCount"),
    winddownTimer = config.getParameter("spawnalienqueenminionAttack.winddownTime")
  }
end

function spawnalienqueenminionAttack.enter()
  if not hasTarget() or currentPhase() < 4 then return nil end

  return spawnalienqueenminionAttack.initialStateData()
end


function spawnalienqueenminionAttack.enterWith(args)
...
--------------------------------------------------------------------------------
function spawnalienqueenminionAttack.enteringState(stateData)
  animator.setAnimationState("movement", "idle")

  monster.setActiveSkillName("spawnalienqueenminionAttack")

  stateData.lastToSpawn = world.distance(self.spawnPosition, mcontroller.position())
end

--------------------------------------------------------------------------------
function spawnalienqueenminionAttack.update(dt, stateData)
  ...
  --In firing position
    ...
    if stateData.skillTimer < stateData.skillDuration then
      ...
      if stateData.fireTimer <= 0 then
        ...
        spawnalienqueenminionAttack.fire(angle, stateData.projectileCount)
...
function spawnalienqueenminionAttack.fire(angle, count)
  local projectileType = config.getParameter("spawnalienqueenminionAttack.projectile.type")
  local projectileConfig = config.getParameter("spawnalienqueenminionAttack.projectile.config")

  if projectileConfig.power then
    projectileConfig.power = projectileConfig.power * root.evalFunction("monsterLevelPowerMultiplier", monster.level())
  end

  local innerRadius = config.getParameter("spawnalienqueenminionAttack.projectile.innerRadius")
...
function spawnalienqueenminionAttack.leavingState(stateData)
