...
function update(args)
    ...
    if not self.disabled and args.moves["jump"] and canAbility(args) then
        ...
        updateAbility(args)
    elseif self.doingAbility or self.disabled then
        self.doingAbility = false
        animator.stopAllSounds("startDash")
        animator.setAnimationState("dashing", "off")
        animator.setParticleEmitterActive("dashParticles", false)
    ...
    if not self.disabled and not self.doingAbility then
        if mcontroller.jumping() then updateJumping(args) end
    ...
    if not self.disabled then updateAlways(args) end
    ...
    elseif self.wall then
        ...
        if not checkWall(self.wall) or
            status.statPositive("activeMovementAbilities") then
            releaseWall()
...
function disabled(args)
    return mcontroller.liquidMovement() or status.statPositive("activeMovementAbilities")
end

function canAbility(args)
    return not mcontroller.jumping() and not mcontroller.canJump() and
        args.moves["jump"] and
        (not (mcontroller.velocity()[2] > 0) or self.doingAbility)
end
...
function updateJumping(args)
    ...
    world.spawnProjectile("dashProjectile", mcontroller.position(), entity.id(), {0, 0}, true, configBombDrop)
...
function updateAlways(args)
    status.setPersistentEffects("movementAbility", {{
            ...
            effectiveMultiplier = self.fallDamageMultiplier
    }})
end
