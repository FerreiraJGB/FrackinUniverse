...
function update(dt)
	if monster or npc then
		self.randScream = math.random(1000)
		if self.randScream >= 999 then
			if monster then
				monster.say("PAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAARTY!!!!!")
			elseif npc then
				npc.say("PAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAARTY!!!!!")
			end
		elseif self.randScream >= 998 then
			if monster then
				monster.say("WOOOOOOOOOOOOOOO THIS IS AWESOME!!!")
			elseif npc then
				npc.say("WOOOOOOOOOOOOOOO THIS IS AWESOME!!!")
			end
		elseif self.randScream >= 997 then
			if monster then
				monster.say("WHY THE HELL AM I SCREECHING SO LOUDLY?")
			elseif npc then
				npc.say("I MAY HAVE JUST SOILED MYSELF IN MY OVEREXUBERANT EXCITEMENT.")
			end
		elseif self.randScream >= 996 then
			if monster then
				monster.say("HEY, WANT A LIGHT SHOW?!?!?!")
			elseif npc then
				npc.say("HEY, WANT A LIGHT SHOW?!?!?!")
			end
		end
	end
