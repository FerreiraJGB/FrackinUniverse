...
function growPlant(growthmod, dt)
	...
	-- Add growth
	storage.growth = storage.growth + getFertSum("growthRate") * (growthmod or 1) * tenantNumber  * dt
	...
	if stage().harvestPool then
		...
		if stage().resetToStage then
			--storage.currentseed.count = getFertSum("yield")--removing due to massive seed gen
...
function fu_isAddonCommunityFarm() return true end
