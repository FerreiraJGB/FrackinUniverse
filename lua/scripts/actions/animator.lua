...
function setAnimationState(args, board)
	...
	local pass,result=pcall(animator.setAnimationState,args.type, args.state)
	if not pass then
		sb.logInfo("animator.lua error: could not set state %s for type %s, for entity type %s of id %s",args.state,args.type,(monster and monster.type()) or (npc and npc.npcType()),entity.id())
		error(result)
	end
	return true
end
