...
function init()
	...
	if not storage.questStarted then
		storage.initCount=(storage.initCount or 0) + 1
		if (storage.initCount > 1) then
			if not (config.getParameter("canBeAbandoned")==false) then
				storage.queueFail=true
			else
				storage.queueStart=true
			end
		end
	end
