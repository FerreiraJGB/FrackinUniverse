...
-- Bow primary ability
NebRNGWarpPoint = WeaponAbility:new()

function NebRNGWarpPoint:init()
...
function NebRNGWarpPoint:update(dt, fireMode, shiftHeld)
...
function NebRNGWarpPoint:uninit()
	self:reset()
end

function NebRNGWarpPoint:reset()
...
function NebRNGWarpPoint:attemptTeleport()
	...
	if self.targetPosition ~= nil then
		...
		if resolvedPoint and self.targetPosition ~= nil then
			...
			if world.magnitude(mcontroller.position(), resolvedPoint) <= self.maxTeleportDistance then
				...
				world.spawnProjectile("rngbowwarppointarrowteleportout", lastPosition)
