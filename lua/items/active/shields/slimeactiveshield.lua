 ...
 if getSkinColorOnce == 0 then
		...
		while i < 12 do
				if string.find(sb.printJson(result[i].image), "head") ~= nil then
					...
					activeItem.setInventoryIcon("/items/active/shields/slimeactiveshield/slimeactiveshield.png:nearidle".."?" .. splited[2])
	...
	if not status.resourcePositive("shieldStamina") and activeNova == 0 then
