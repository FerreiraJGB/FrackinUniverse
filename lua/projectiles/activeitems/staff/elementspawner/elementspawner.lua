...
function init()
  ...
  self.control = config.getParameter("control")
  ...
  message.setHandler("updateProjectile", function(_, _, aimPosition, facingDirection)
...
function processTimedAction(action, dt)
  --need to tweek direction and offset since they move diagonally
  action.direction[1] = self.facingDirection
  action.offset[1] = -30 * self.facingDirection
